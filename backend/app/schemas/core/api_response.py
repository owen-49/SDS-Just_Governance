# backend/app/schemas/core/api_response.py
...
def ok(
    ...
    """
    payload = ApiResponse(
        code=BizCode.OK, message=message, data=data, request_id=_get_rid(request)
    ).model_dump()
...
def fail(
    *,
    ...
    """
    payload = ApiResponse(
        code=code, message=message, data=data, request_id=_get_rid(request)
    ).model_dump()
