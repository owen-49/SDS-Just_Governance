...
export const dbApi = {
  ...
  createEmailVerificationToken(email) {
    ...
    const token = Math.floor(100000 + Math.random() * 900000).toString(); // 6位数字验证码
    const expires_at = new Date(Date.now() + 10 * 60 * 1000).toISOString(); // 10分钟过期
    db.email_verification_tokens[email] = { token, expires_at, used_at: null };
    writeDB(db);
    console.log(`Verification code for ${email}: ${token}`); // 开发环境显示验证码
  ...
  consumeEmailVerificationToken(token) {
    ...
    // 查找匹配的token
    for (const [email, data] of Object.entries(db.email_verification_tokens)) {
      if (data.token === token && !data.used_at) {
